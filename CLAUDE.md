...
- **개인 할당은 설정값이 아니라 "잔여값"**: `(총예산 − 공용 카테고리 합 − 개인 조정 합) ÷ N`, N=팀원 수(가변).
- **월(YYYY-MM) 단위** 운영: 매월 1일 한도 재부여, 미사용액 **이월 없이 소멸**. 카테고리·조정·인원은 **월 시작 스냅샷**으로 고정 → 과거 월 집계 불변.
...
- **프로젝트 스펙** (`ref-docs/specs/` 하위) — 📝 자유롭게 작성. → [SDD 가이드라인](ref-docs/claude/sdd.md) · `/spec-guard`로 정합성 분석
  - 현재 스펙: [부서비 운영 관리 웹앱 PRD](ref-docs/specs/design/부서비-운영관리-웹앱-PRD.md) (design, v0.3.0)
