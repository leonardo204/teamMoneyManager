...
| 탭 | 기능 |
|----|------|
| **대시보드** | 회계 월 선택(선택 시 대시보드·내역 동기화), 카테고리별 배정·사용·잔액 카드(소진 상태 배지), 팀원별 할당·사용·잔액 표, 전체 소진율 |
| **설정** | 팀원 관리(추가·수정·비활성/삭제), 공용 카테고리 단일 관리(balancing 자투리는 서버 확정 allocation 기준 표시 → 대시보드와 동기화), 인당 금액·개인 예산·자투리 흡수 카테고리(balancing) 설정, 개인 조정(±), 개인 할당 미리보기(계산식 공개: 총예산·개인예산 합·자투리·balancing 최종의 산식과 실제 값), **비밀번호 변경**. 개인 예산 저장으로 자투리가 음수가 되면 모달로 카테고리를 즉시 줄이도록 안내 |
| **내역** | 지출 목록·필터(분류·팀원), 수정·삭제, 변경 이력(감사 로그) |

지출 입력은 어느 화면에서든 헤더의 **"+ 지출 입력"** 버튼(빠른 금액 버튼·공용/개인 토글)으로. 헤더는 스크롤 시 상단에 **고정(sticky)** 됩니다.
...
| `GET /api/dashboard` | 카테고리·팀원 잔액·소진율 |
| `GET /api/audit-logs` | 변경 이력 |

> `transactions.card`(1|2) 컬럼과 관련 API 파라미터(`card`)는 하위호환·기존 데이터 보존을 위해 스키마·엔드포인트에 남아 있으나 **UI에서는 더 이상 사용하지 않습니다**(신규 지출은 `card=null`).
