...
단일 공유 계정으로 로그인해, 매월 예산을 카테고리(공용)와 개인 할당으로 나누고, 지출을
기록하며 잔액을 한눈에 확인합니다. 외부 SaaS 의존 없이 단일 VM에서 `docker compose`로
자체 호스팅합니다.
...
**1. 개인 할당은 "설정값"이 아니라 "잔여값"이다.**

```
총예산        = 인당 금액(180,000) × 팀원 수 N        (N은 가변)
나눌 잔액     = 총예산 − 공용 카테고리 합 − 개인 조정 합
개인 기본할당 = 나눌 잔액 ÷ N          (정수 floor + 나머지 1원씩 분배)
팀원별 최종   = 개인 기본할당 + 본인 조정액
```

공용 카테고리(회식·간식 등)를 늘리면 개인 할당이 줄고, 줄이면 늘어납니다. 어떤 설정·인원
조합에서도 **`공용 합 + 팀원별 최종 합 = 총예산`이 정확히 성립**합니다(돈이 새거나 남지 않음).
생일축하금 등은 공용이 아니라 **개인 조정(±) 라인**으로 처리합니다.
...
| **설정** | 개인 할당 실시간 미리보기, 팀원 관리(추가·수정·비활성/삭제), 공용 카테고리(템플릿·당월 금액)·인당 금액, 개인 조정(±), **비밀번호 변경** |
...
- 팀원 증감 시 당월 총예산·개인 할당 자동 반영 (지출 있는 팀원은 삭제 대신 비활성)
...
| `GET/POST/PUT/DELETE /api/category-templates` | 월 시드용 기본 카테고리 |
...
| `GET /api/allocation` · `POST /api/allocation/preview` | 개인 할당 계산·실시간 미리보기 |
...
초기 시드: `settings.per_person = 180000`, `category_templates` = 야근 400,000 / 커피 210,000 / 간식 100,000 / 회식 100,000.
